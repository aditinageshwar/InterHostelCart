...
const Auction = {
  createAuction: async (itemId, startingBid, endTime) => {
    try {
      const sqlInsert = "INSERT INTO auctions (itemId, startingBid, endTime) VALUES (?, ?, ?)";
      const [result] = await pool.query(sqlInsert, [itemId, startingBid, endTime]);
      return result;
    } 
    catch (error) {
      throw error;
  ...
  placeBid: async (auctionId, userId, bidAmount) => {
    ...
    try {
      ...
      await connection.query("LOCK TABLE bids WRITE");
  ...
  getAuctionByItemId: async (itemId) => {
    try {
      const sqlSelect = "SELECT auctionId FROM auctions WHERE itemId = ?";
